...
<body background="img/bg.jpg" style="background-size:cover; opacity:1">
	...
	<?php
		...
		if(isset($_GET['id']) && isset($_GET['s'])) {
			...
			$kd = "J".$idp."-".$ido."";
			//echo "<script>Materialize.toast('Data diperbaharui', 1000)</script>;";
			$que="INSERT INTO `order_jahit_1` (`kdbooking`, `idpelanggan`, `alamat`, `kategori`, `ukuran`, `harga`, `tglorder`, `tglkirim`, `jumlah`, `kain`, `desain`) 
			VALUES ('$kd', '$idu', '$alamat', '$kategori', '$ukuran', '$total', '1', '1', '300000', '3', '2018-03-29', '2018-04-25', '4', '4', 'meetjahit.com/j12');";
			$pro=mysqli_query($con,$que);
			while ($dat = mysqli_fetch_array ($pro)){
				$price=$dat['tarif'];
			}
			...
			//if($pro) header('location:index.php');
		}
	?>
