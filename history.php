...
<body background="img/bg.jpg" style="background-size:cover; opacity:1">
	...
	<!--UI Web-->	
		...
		<div class="col l12 m12 s12" style="background-color: rgba(104, 73, 50,0.7); margin-top:-20px; margin-left:80px; margin-right:80px; text-align:center; padding:40px;">
			...
			$myquery="SELECT * FROM tb_history WHERE iduser ='$idu' order by idhis DESC";
			...
			while ($beli = mysqli_fetch_array ($myprocess)){
				...
				$myq="SELECT * FROM order_jahit_".$idp." WHERE kdbooking ='$kd' order by tglorder";
				...
				while ($his = mysqli_fetch_array ($myp)){
					...
					echo"
						...
						";
								...
								while ($user = mysqli_fetch_array ($p)){
									echo"<div style='padding-top:4px; font-size:16px;'>Order kepada: ".$user['nama']."</div>";	
								}							
						echo"
					...
					else if(($kirim==3)&&(($rat==NULL)||($rat==0))){
						$status="Konfirmasi pengiriman";
						echo"
					...
					else if(($his['bayar']==1)&&($his['progress']==1)&&(($his['kirim']==0)||($his['kirim']==2))){
					...
					echo"
						<a href='histodetail.php?kd=".$kd."&idp=".$idp."' style='padding-top:8px; font-size:18px; color:rgba(0,255,255,1)' text-align:right;>Lihat rincian>>></a>
						</div>
						</div>
					";
				}
